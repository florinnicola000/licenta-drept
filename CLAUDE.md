...
- **Defalcare pe zile:** fiecare sarcină primește un badge cu ziua (Luni → Sâmbătă, Duminică = odihnă), atribuit secvențial prin `taskDay(i)` din `STUDY_DAYS` (a 1-a sarcină = Luni, a 2-a = Marți...). Săptămânile care își descriu deja zilele în text (ex. W32 examen: „Ziua 1", „Zilele 1-2") sunt detectate cu regex `/\bZiua\b|\bZilele\b/` și NU primesc badge auto (`ownDays`).
